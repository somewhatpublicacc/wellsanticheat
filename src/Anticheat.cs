...
namespace WellsAntiCheat
{
    ...
    internal static class Anticheat
    {
        ...
        private static bool SenderIsHost(PlayerControl player)
            => player != null && AmongUsClient.Instance != null && player.OwnerId == AmongUsClient.Instance.HostId;
        ...
        private static bool HandleRpc(Type sourceNetObj, PlayerControl player, byte callId, MessageReader reader)
        {
            ...
            if (player != null)
            {
                ...
                // 4. State-based rejection (cosmetics mid-game, gameplay-in-lobby, host-only).
                if (!blockRpc)
                    StateChecks.Check(player, callId, SenderIsHost(player), ref blockRpc);
            }
